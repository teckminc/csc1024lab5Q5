...
def test_case4(monkeypatch, capsys):
    ...
    assert captured_stdout.strip().find(f'[5.0, \'KualaLumpur\', \'Malaysia\']') != -1
    assert captured_stdout.strip().find(f'[5.0, \'KualaLumpur\']') != -1
    assert captured_stdout.strip().find(f'[5.0]') != -1
